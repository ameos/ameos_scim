...
class UserService
{
    ...
    public function search(array $queryParams, array $configuration): array
    {
        ...
        [$totalResults, $result] = $this->frontendUserRepository->search(
            $queryParams,
    ...
    public function create(array $payload, array $configuration): array
    {
        ...
        $user = $this->frontendUserRepository->create($data, (int)$configuration['pid']);
    ...
    {
        $this->frontendUserRepository->delete($userId);
        $this->eventDispatcher->dispatch(new PostDeleteUserEvent($userId, Context::Frontend));
    }
